...
const BookDetail = () => {
  ...
  const [rating, setRating] = useState(0); // 保存用户的评分
    ...
    <div className="w-1/4 bg-[#c6a89a] p-4 mr-8 rounded-md flex flex-col items-center">
        ...
        <div className="flex justify-center items-center mb-2">
            ...
            <span
            key={index}
            className={`text-2xl cursor-pointer ${index < rating ? 'text-[#815f55]' : 'text-gray-400'}`}
              onMouseEnter={() => setRating(index + 1)} // 当鼠标滑过时改变评分
              onClick={() => setRating(index + 1)} // 当用户点击时保存评分
            >
            ★
            </span>
            ))}
